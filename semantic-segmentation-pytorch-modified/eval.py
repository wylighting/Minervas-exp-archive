...
from mit_semseg.utils import AverageMeter, colorEncode, accuracy, intersectionAndUnion, setup_logger, f_measure
...
labels = np.load('data/labelids.npy', allow_pickle=True).item()
...
def evaluate(segmentation_module, loader, cfg, gpu):
    ...
    for batch_data in loader:
        ...
        # if cfg.DATASET.list_val == 'S3D_testing.odgt':
#        if cfg.DATASET.list_val == 'MYDATASS_validation.odgt':
        boundary = f_measure(pred, seg_label)
        boundary_meter.update(boundary)
    ...
    for i, _iou in enumerate(iou):
        print('class [{}]/[{}], IoU: {:.4f}'.format(i+1, labels.get(str(i+1), 'None'), _iou*100))

    print('[Eval Summary]:')
    print('Mean IoU: {:.2f}, Accuracy: {:.2f}, Boundary: {:.2f}'.format(
            iou.mean()*100, acc_meter.average()*100, boundary_meter.average()*100))
    # else:
        # eval summary
    #    print('[Eval Summary]:')
