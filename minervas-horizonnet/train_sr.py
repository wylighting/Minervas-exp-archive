...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_root_dir', default='data/mp3d/train',
                        help='root directory to training dataset. '
                             'should contains img, label_cor subdirectories')
    parser.add_argument('--valid_root_dir', default='data/mp3d/valid',
    ...
    parser.add_argument('--epochs', default=291, type=int,
    ...
    # create two dataloader
    dataset_real_train = PanoCorBonDataset(
        root_dir="data/mp3d/train",
        flip=not args.no_flip, rotate=not args.no_rotate, gamma=not args.no_gamma,
        stretch=not args.no_pano_stretch)
    loader_real_train = DataLoader(dataset_real_train, 16,
                              shuffle=True, drop_last=True,
                              num_workers=args.num_workers,
                              pin_memory=not args.no_cuda,
                              worker_init_fn=lambda x: np.random.seed())

    dataset_syn_train = PanoCorBonDataset(
        root_dir="data/Minervas_Layout_filtered_final/",
        flip=not args.no_flip, rotate=not args.no_rotate, gamma=not args.no_gamma,
        stretch=not args.no_pano_stretch)
    loader_syn_train = DataLoader(dataset_syn_train, 8,
                              shuffle=True, drop_last=True,
                              num_workers=args.num_workers,
                              pin_memory=not args.no_cuda,
                              worker_init_fn=lambda x: np.random.seed())
    ...
    # Init variable
    args.warmup_iters = args.warmup_epochs * len(loader_real_train)
    args.max_iters = args.epochs * len(loader_real_train)
    args.running_lr = args.warmup_lr if args.warmup_epochs > 0 else args.lr
    args.cur_iter = 0
    args.best_valid_score = 0

    iterator_syn_train = iter(loader_syn_train)
    ...
    for ith_epoch in trange(1, args.epochs + 1, desc='Epoch', unit='ep'):
        ...
        iterator_real_train = iter(loader_real_train)
        for _ in trange(len(loader_real_train),
                        desc='Train ep%s' % ith_epoch, position=1):
            # Set learning rate
            adjust_learning_rate(optimizer, args)

            args.cur_iter += 1
            x_r, y_bon_r, y_cor_r = next(iterator_real_train)

            # train syn
            try:
                x_s, y_bon_s, y_cor_s = next(iterator_syn_train)
            except StopIteration:
                iterator_syn_train = iter(loader_syn_train)
                x_s, y_bon_s, y_cor_s = next(iterator_syn_train)
            # combine
            x = torch.cat((x_r, x_s), dim=0)
            y_bon = torch.cat((y_bon_r, y_bon_s))
            y_cor = torch.cat((y_cor_r, y_cor_s))
