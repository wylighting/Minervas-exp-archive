    ...
    for i, data in enumerate(train_dataloader):
        ...
        if step % cfg.TRAIN.VAL_STEP == 0 and step != 0 and val_dataloader is not None:
            ...
            if val_err[0]['abs_rel'] < val_err_lowest:
                ...
                save_ckpt(train_args, step, epoch, model, optimizer.optimizer, scheduler, val_err[0], is_best=True)
            # training mode
            model.train()
...
if __name__=='__main__':
    ...
    try:
        ...
        for epoch in range(train_args.start_epoch, train_args.epoch):
            # training
            val_err_lowest = train(train_dataloader, model, epoch, loss_func, optimizer, scheduler, training_stats, val_err_lowest,
