...
if __name__ == '__main__':
    ...
    for i, data in enumerate(data_loader):
        ...
        # tensor([96,  0,  0,  0]) torch.Size([289, 385]) tensor([1.6623]) torch.Size([480, 640])
        # print(invalid_side, pred_depth.shape, data['ratio'], torch.squeeze(data['B_raw']).shape)
        # pred_depth = pred_depth / data['ratio'].cuda() # scale the depth
        pred_depth = resize_image(pred_depth, torch.squeeze(data['B_raw']).shape)
        smoothed_criteria, smoothed_criteria_instance = evaluate_err(pred_depth, data['B_raw'], smoothed_criteria, mask=(45, 471, 41, 601), scale=10.)
        ...
        # Add code
        print("absREL ERROR: %f",smoothed_criteria_instance['err_absRel'])
        print("log10 ERROR: %f",smoothed_criteria_instance['err_log10'])
        print("RMS ERROR: %f",smoothed_criteria_instance['err_rms'])
        if smoothed_criteria_instance['err_absRel'] < 0.335 and smoothed_criteria_instance['err_log10'] < 0.127 and smoothed_criteria_instance['err_rms'] < 1.06:
            smoothed_criteria_instance['img_path'] = img_name
            metric_list.append(smoothed_criteria_instance)
        total_metric = 2 * smoothed_criteria_instance['err_absRel'] + 3 * smoothed_criteria_instance['err_log10'] + smoothed_criteria_instance['err_rms']
        with open(os.path.join(image_dir, img_name.replace('.png', '.txt')), 'w') as f:
    ...
    with open("metric_output.json", "w") as f:
        print("Good data num : ", len(metric_list))
