...
def train(model, args, epochs=10, experiment_name="DeepLab", lr=0.0001, root=".", device=None,
    ...
    train_synthetic_loader = DepthDataLoader(args, 'synthetic').data
    ...
    if args.resume != '':
        model, optimizer, args.epoch = model_io.load_checkpoint(args.resume, model, optimizer=optimizer)
    ################################################################################################
    # some globals
    ...
    iter_syn_train = iter(train_synthetic_loader)
    ...
    for epoch in range(args.epoch, epochs):
        ...
        for i, batch in tqdm(enumerate(train_loader), desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Train",
            ...
            ## Add synthetic data to the batch
            try:
            ...
            batch['image'] = torch.cat((batch['image'], syn_data['image']), dim=0)
            batch['depth'] = torch.cat((batch['depth'], syn_data['depth']), dim=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bs', default=10, type=int, help='batch size') # for real+synthetic training
    ...
    parser.add_argument("--data_path", default='../datasets/nyu_depth_v2/official_splits/train', type=str,
                        help="path to dataset")
    parser.add_argument("--gt_path", default='../datasets/nyu_depth_v2/official_splits/train', type=str,
                        help="path to dataset")

    parser.add_argument("--syn_data_path", default='../datasets/MYDATA_new_depth/dataset', type=str,
                        help="path to dataset")
    parser.add_argument("--syn_gt_path", default='../datasets/MYDATA_new_depth/dataset', type=str,
                        help="path to dataset")
    ...
    parser.add_argument('--data_path_eval',
                        default="../datasets/nyu_depth_v2/official_splits/test/",
                        type=str, help='path to the data for online evaluation')
    parser.add_argument('--gt_path_eval', default="../datasets/nyu_depth_v2/official_splits/test/",
