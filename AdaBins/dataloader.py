...
class DepthDataLoader(object):
    def __init__(self, args, mode):
        if mode == 'train':
            ...
            self.data = DataLoader(self.training_samples, 6,
                                   shuffle=(self.train_sampler is None),
                                   num_workers=6,
                                   pin_memory=True,
                                   sampler=self.train_sampler)
...
class DataLoadPreprocess(Dataset):
    def __init__(self, args, mode, transform=None, is_for_online_eval=False):
        ...
        if mode == 'online_eval':
            ...
            with open('./train_test_inputs/nyudepthv2_final_syn_train_files_with_gt.txt', 'r') as f:
                self.filenames = f.readlines()
        else:
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train' or self.mode == 'synthetic':
            ...
            if self.mode == 'synthetic':
                image_path = os.path.join(self.args.syn_data_path, remove_leading_slash(sample_path.split()[0]))
                depth_path = os.path.join(self.args.syn_gt_path, remove_leading_slash(sample_path.split()[1]))
